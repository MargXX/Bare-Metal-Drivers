...
|BMP390 (device) |Complete   |Verified on hardware: init, configure, soft reset, data-ready, forced and normal reads, float compensation. Ambient readings 101,150 Pa and 23 °C.|
...
|Layer                         |Status     |Scope                                                                          |
|------------------------------|-----------|-------------------------------------------------------------------------------|
|1. Host-side unit tests       |Partial    |Driver logic, compiled and run natively. No hardware.                          |
|2. Hardware-in-the-loop (HIL) |Planned    |The driver against the physical sensor over a real bus.                        |
|3. Continuous integration     |Complete   |Cross-compile and host test suite on every push.                               |

### Layer 1: host-side unit tests

The BMP390 compensation math is separated from bus I/O inside the driver, so it can be exercised without I2C. `tests/bmp390_test_host.c` includes `bmp390.c` directly, placing both in a single translation unit so the file's internal `static` helpers are reachable from the test. It links against [Unity](https://github.com/ThrowTheSwitch/Unity), vendored under `tests/unity/`. Symbols that the rest of the driver references but these tests never call (`bm_i2c_*`, `bm_systick_*`) are resolved by stubs in `tests/stubs.c`.

**Implemented.** Tests for `power_of_two`, `compensate_temperature`, and `compensate_pressure`, using hand-computed expected values. Each formula group is tested with the other coefficients zeroed, so a failure identifies a single term, and again in combination to check that the groups sum correctly. Negative coefficients are covered, as are cases confirming that the temperature and pressure coefficient sets do not affect each other. The suite is registered with CTest.

**Planned.** A fake I2C backend returning canned register contents, which enables tests for calibration parsing and the read-and-compensate path. The reference data is a 21-byte calibration blob captured from the physical BMP390.

### Layer 2: hardware-in-the-loop

Planned. A `pytest` harness driving the RP2040 over UART via an ASCII command protocol, checking register contents, configuration round-trips, and sensor output ranges against the physical device.
